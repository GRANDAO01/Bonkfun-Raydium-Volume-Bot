...
const keypairsDir = "src/raydiumcpmm/keypairs";
...
export async function extender_token(config: any = null) {
	...
	for (let i = 0; i < cycles; i++) {
		...
		try {
			...
			const wsolnamount = await getSwapQuote(buyAmount, marketPk.toBase58());
			const solnamount = wsolnamount / (10 ** 9)
			await executeSwaps(keypairs, blockhash, solnamount, marketPk);
		} catch (error) {
			console.error(chalk.red("Error executing swaps:"), error);
		}
