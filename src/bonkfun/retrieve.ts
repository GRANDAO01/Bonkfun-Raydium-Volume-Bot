...
import { burnAccount, checkMintKey, getSwapInstruction, sell } from "./utils";
...
export async function closeAcc() {
	...
	// const tokenMint = "8pGqjEVBnmxmf6mo5DkzieoNWVoqre5vsFDC59K8jk2w";
	const isValidPubkey = await checkMintKey(tokenMint);
	if (!isValidPubkey) {
	...
	const delaySellIn = parseInt("100", 10);

	if (!fs.existsSync(keypairsPath)) {
		console.log(chalk.red(`No keypairs found for Pair ID/	: ${tokenMint}`));
		process.exit(0);
	}
	...
	if (jitoTipAmtInput) {
		...
		if (tipValue >= 0.01) {
			console.log(chalk.red("Error: Tip value is too high. Please enter a value less than or equal to 0.01."));
			process.exit(0);
		}
	} else {
		...
		for (let i = 0; i < keypairs.length; i++) {
			...
			if (tokenAccountExists || wsolAccountExists || solBalance > minBalanceForRent) {
				...
				if (tokenAccountExists) {
					...
					const sellInstruction = await getSwapInstruction(Number(tokenBalance.value.uiAmountString), 0, swapAccountKey, tokenKey);
...
export async function closeSpecificAcc(keypairs: Keypair[], mint: string, jitoTip: number, block: string | Blockhash) {
	...
	for (let i = 0; i < keypairs.length; i++) {
		...
		if (tokenAccountExists) {
			...
			// Burn token account
			let baseTokenBurnInstruction = spl.createCloseAccountInstruction(tokenAcc, wallet.publicKey, keypair.publicKey);
			//let baseTokenBurnInstruction1 = spl.createCloseAccountInstruction(tokenAcc, wallet.publicKey, keypair.publicKey);
			//instructionsForChunk.push(baseTokenBurnInstruction);
			instructionsForChunk.push(baseTokenBurnInstruction);
		}
