 ...
 */
export async function extender_token(config: any = null) {
	// console.log("11111111111111111111111111111111111111\n")
	console.clear();
	console.log(chalk.green("\n==================== Buy Step of token purchage unit ===================="));
	...
	if (config) {
		...
		minAndMaxBuy = prompt(chalk.yellowBright("TOKEN: Enter the amount of min and max amount you want to BUY (syntax: MIN_AMOUNT MAX_AMOUNT): "));
		...
		minAndMaxSell = prompt(chalk.yellowBright("TOKEN: Enter the amount of min and max amount you want to Sell(syntax: MIN_AMOUNT MAX_AMOUNT): "));
		isMaxMinValid = isValidTwoNumberInput(minAndMaxSell);

		if (!isMaxMinValid) {
			console.log(chalk.red("Error: Invalid input. Please enter a sell amount input. exam:0.001 0.002"));
			process.exit(0x0);
		}
		...
		if (!isMaxMinValid) {
			console.log(chalk.red("Error: Invalid input. Please enter a wallet amount input. exam:1 2"));
			process.exit(0x0);
		}

		delayIn = prompt(chalk.cyan("Min and Max Delay between swaps in seconds Example MIN_DELAY MAX_DELAY: "));
		isMaxMinValid = isValidTwoNumberInput(delayIn);
		...
		const isValidNumber = isPositiveInteger(cyclesIn);
	...
	for (let i = 0; i < cycles; i++) {
		...
		///////////////////////////////////////////////////////////////////////////
		const a1 = await getSwapQuote(buyAmounts[0], baseMint.toBase58(), spl.NATIVE_MINT.toBase58());
		const a2 = await getSwapQuote(buyAmounts[1], baseMint.toBase58(), spl.NATIVE_MINT.toBase58());
		...
		console.error(chalk.red("buy amount-- 22 -----------------"), a2);
		console.error(chalk.red("buy amount-- 22 -----------------"), buyAmount);
		console.error(chalk.red("buy amount-- 22 -----------------"), sellAmount);
		...
		try {
			...
			if (baseMint) {
				...
				await executeSwaps(keypairs, jitoTipAmt, blockhash, buyAmount/1e9, baseMint);
			}
		} catch (error) {
		...
		for (const file of files) {
			...
			if (sellKeypairs.size >= (sellAmount/1e9)) break; // Limit to specified sellAmount per cycle
		}
...
export async function extender_sol(config: any = null) {
	console.clear();
	console.log(chalk.green("\n==================== Buy Step of sol purchage unit ===================="));
	console.log(chalk.yellow("Follow the instructions below to perform the buy step.\n"));
	...
	for (let i = 0; i < transactions.length; i++) {
		try {
			...
			if (confirmation.value.err) {
				...
				console.log(`Transaction ${i + 1} confirmed successfully https://solscan.io/tx/${signature}`);
			}
